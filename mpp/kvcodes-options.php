<?php 
/*
Plugin Name: Custom Options Tabbed Page - Kvcodes
Plugin URI: http://www.kvcodes.com/?p=829&preview=true
Description: This is just a plugin,  demonstrate howto create custom options with amdin setting page. 
Author: Varadharaj V
Version: 1.0
...
class KvcodesOptionsPage {
    ...
    public function add_plugin_page(){
     ...
     public function kv_admin_kvcodes() {
        ...
        $social_Screen = ( isset( $_GET['tab'] ) && 'social' == $_GET['tab'] ) ? true : false;
        $footer_Screen = ( isset( $_GET['tab'] ) && 'footer' == $_GET['tab'] ) ? true : false;   ?>
        <div class="wrap">
            <h1>Kvcodes Settings</h1>
            <?php settings_errors(); ?><h2 class="nav-tab-wrapper">
            
				<a href="<?php echo admin_url( 'admin.php?page=kvcodes' ); ?>" class="nav-tab<?php if ( ! isset( $_GET['tab'] ) || isset( $_GET['tab'] ) && 'social' != $_GET['tab']  && 'footer' != $_GET['tab'] ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'General' ); ?></a>
				<a href="<?php echo esc_url( add_query_arg( array( 'tab' => 'social' ), admin_url( 'admin.php?page=kvcodes' ) ) ); ?>" class="nav-tab<?php if ( $social_Screen ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'Social' ); ?></a> 
				<a href="<?php echo esc_url( add_query_arg( array( 'tab' => 'footer' ), admin_url( 'admin.php?page=kvcodes' ) ) ); ?>" class="nav-tab<?php if ( $footer_Screen ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'Footer' ); ?></a>        
			</h2>
...
if( is_admin() )
 ...
 wp_enqueue_script('rbr-jquery', plugins_url('mpp/admin_js.js', dirname(__FILE__)),  array('jquery'), '1.0', true);
